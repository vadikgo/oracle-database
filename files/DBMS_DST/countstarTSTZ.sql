...
Rem
Rem    NAME
Rem     countstarTSTZ.sql - Gives count of TimeStamp with TimeZone columns using an explicit count(*)
Rem     if your stats are up to date then use countstatsTSTZ.sql which will use stats info (faster)
Rem		Version 1.4
Rem
Rem    NOTES
...
Rem      in an database using a count(*) fo each table that has a TSTZ column.
...
begin
		...
		LOOP
			...
			THEN
				...
				IF V_COUNTSTAR > 0 THEN
					DBMS_OUTPUT.PUT_LINE(REC.OWNER ||'.'|| REC.TABLE_NAME || '.'|| REC.COLUMN_NAME ||' - ' || V_COUNTSTAR );
				END IF;
			ELSE
				IF V_COUNTSTAR > 0 THEN
					DBMS_OUTPUT.PUT_LINE(REC.OWNER ||'.'|| REC.TABLE_NAME || '.'|| REC.COLUMN_NAME ||' - ' || V_COUNTSTAR );
	...
	DBMS_OUTPUT.PUT_LINE( ' Total count * of SYS TSTZ columns is : ' || TO_CHAR(V_TOTALCOUNT) );
...
begin
	...
	DBMS_OUTPUT.PUT_LINE( ' Total count * of non-SYS TSTZ columns is :  ' || TO_CHAR(V_TOTALCOUNT) );
